...
### [Adlerweg Tirol](https://webmapping.github.io/adlerweg/index.html)
Javascript-Objekte als Datengrundlage; Map Icons Collection; *L.icon()* mit *iconUrl* und Positionierung über *iconSize*, *iconAnchor*, *iconPopup*; *leaflet-gpx* Plugin von `mpettazoni`; HTML-Elemente mit *.querySelector* und *.innerHTML* durch Werte aus dem Skript ersetzen; Navigations-Pulldownmenü aus einem Javascript-Objekt dynamisch erstellen; Javascript: Schleifen mit *for of* (Arrays) und *for in* (Objekte); *String.replace("a","b")* und *String.replace(/a/g,"b")* (i.e. reguläre Ausdrücke / global)
